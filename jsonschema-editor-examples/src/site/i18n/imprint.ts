...
} as const satisfies Record<"de" | "en", ImprintContent>;

export function imprintFor(locale: AppLocale): ImprintContent {
  const resolved =
    content[locale as keyof typeof content] ?? (locale === "de" ? content.de : content.en);
  return resolved as ImprintContent;
}
