...
export function listUiAttributeFields(
  ...
  if (element instanceof Group) return withOptionalI18n([...GROUP_ATTRIBUTES], options?.includeI18n);
  if (element instanceof Category) return withOptionalI18n([...CATEGORY_ATTRIBUTES], options?.includeI18n);
  if (element instanceof Step) return withOptionalI18n([...STEP_ATTRIBUTES], options?.includeI18n);
  if (element instanceof Label) return withOptionalI18n([...LABEL_ATTRIBUTES], options?.includeI18n);
  return [];
}
